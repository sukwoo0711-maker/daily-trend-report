...
| 날짜 | 리포트 | 비고 |
|---|---|---|
| 2026-08-02 | [보안 1건 + 채택 1건](reports/2026-08-02/README.md) | **07-31 캠페인이 확대됨** — 동일 블롭 `79d0513…`이 **7번째 저장소 `ratchet`**(406★·83fork, 07-31 생성, 이 리포트 주제인 "규칙 준수 검사"를 표방)에 5번째 이름으로 등장. 07-31과 달리 **설치 스크립트가 은닉 실행**(정책 우회·base64 명령·창 숨김·오류 무시)임을 소스로 확인. **지목 계정은 재스윕 목록에 고정.** 08-01 HN ID 규칙 첫 실전에서 양방향 작동(`qm` 623pt 확인 / 시크릿 항목 제외). 게이트 2차 교정 = **불투명 바이너리 vs 읽을 수 있는 스크립트** 분리; 채택 S 1건 kill-ai-slop |
