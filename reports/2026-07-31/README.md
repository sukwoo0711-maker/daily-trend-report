...
## 📝 발행 후 정정 (2026-07-31, 기준 개정)

이 리포트는 아래 open query 1·2를 "판단 필요"로 올린 채 발행됐다. **발행 당일 사용자 판단으로 기준이 개정됐고, 그에 따라 판정 하나가 뒤집혔다.** [METHODOLOGY.md](../../METHODOLOGY.md)의 정정 원칙에 따라 원래 판정을 지우지 않고 함께 남긴다.

**개정 내용** → [ADOPTION-CRITERIA.md](../../ADOPTION-CRITERIA.md)
- **G3 통과 목록을 산출물 유형별로 분리.** 문서형 산출물(마크다운 스킬·프롬프트)에 한해 **CC0-1.0·CC-BY-4.0을 통과**로 인정한다. **CC-BY-NC(비영리 전용)·ND(개작 금지)·SA(동일조건 공개)는 계속 제외** — 회사 업무 사용과 조건이 충돌하거나 스킬 수정이 불가능하기 때문이다. 코드 산출물의 통과 목록은 종전대로(MIT·Apache-2.0·BSD·ISC, CC 계열 전부 제외).
- **`NOASSERTION`은 자동 탈락이 아니라 LICENSE 원문 확인 신호**로 명문화(open query 2).
- **G1 바이너리 스캔을 계정 단위로 승격**해 절차·판별 요령을 기준 문서에 고정(#1의 후속).

**뒤집힌 판정 — `lopopolo/harness-engineering`: 제외 → 채택(문서형 S)**

새 기준으로 재판정하며 실측을 다시 돌렸다(2026-07-31 관측): **2,408★, CC-BY-4.0, 바이너리 스캔 CLEAN, 소유자 계정 전체 스캔도 CLEAN**, 구성은 `md` 48 + `mdx` 15로 사실상 문서 저장소다. 라이선스 처리도 모범적이다 — `COPYING.md`가 **경로별로 조건을 나눠 명시**하고(저자 원문은 CC BY 4.0, `sources/raw/acp/`는 Apache-2.0으로 재라이선스하지 않음), 인용문·이미지·상표는 권리자에게 남는다고 밝히며, 요구 귀속 문구까지 제공한다.

**단, 취할 범위는 문서로 한정한다.** `sources/scripts/` 아래 `.py` 5개는 코퍼스 수집·검증용 도구인데 `COPYING.md`의 CC BY 4.0 문구가 "산문·편집 구성·다이어그램·문서"를 대상으로 하고 **코드용 라이선스를 따로 명시하지 않는다.** 개정 기준의 혼합 저장소 규칙(문서만 인정, 코드는 별도 확인, 확인 안 되면 가져오지 않음)에 따라 **문서만 취하고 스크립트는 가져오지 않는다.**

**부수 의무**: CC-BY 채택분은 출처 표기가 조건이다. 파일 상단에 한 줄 남기면 된다 — `<!-- 출처: Ryan Lopopolo, Harness Engineering, CC BY 4.0, https://github.com/lopopolo/harness-engineering -->`. 수정해서 쓰면 "수정함"도 함께 표시한다.

이 항목의 **내용 심사는 아직 하지 않았다** — 게이트 통과만 확인했다. 실제 유용성 평가는 다음 판으로 넘긴다.
...
| `lopopolo/harness-engineering` (2,407★, 07-18) | ~~**제외(판정 보류)**~~ → **정정: 채택(문서형 S)** | 발행 시점 판정은 **G3** — CC-BY-4.0이 통과 목록(MIT·Apache-2.0·BSD·ISC) 밖. **발행 후 기준 개정으로 뒤집힘** — 아래 [발행 후 정정](#-발행-후-정정-2026-07-31-기준-개정) 참조 |
